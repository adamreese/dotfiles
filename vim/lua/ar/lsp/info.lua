...
local function make_client_info(client)
  ...
  local capabilities = vim.inspect(client.resolved_capabilities)
  info_lines = vim.list_extend(info_lines, vim.split(capabilities, '\n'))
...
function M.active_lsp()
  ...
  vim.pretty_print(active)
end

function M.buf_active_lsp()
  local active = vim.tbl_map(function(client)
    return { name = client.name, id = client.id }
...
function M.buffer_active_lsp_capabilities()
  ...
  local active = vim.tbl_map(function(client)
    vim.pretty_print(client.resolved_capabilities)
  end, vim.lsp.buf_get_clients(0))
end
